...
function sanitizeObject(
  ...
  for (const [key, value] of Object.entries(config)) {
    ...
    if (isSensitiveField(key) && typeof value === 'string') {
      ...
      // Special handling for accounts array - sanitize keys inside each account
      sanitizedObj[key] = value.map((account: any) => {
        if (typeof account === 'object' && account !== null && account.key) {
          return {
            ...account,
            key: `\${${generateEnvVarName('global', config.name || 'ACCOUNT', 'API_KEY')}}`
          };
        }
        return account;
      });
      sanitizedCount += value.filter((acc: any) => acc && acc.key).length;
