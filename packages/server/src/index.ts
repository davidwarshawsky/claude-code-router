...
async function getServer(options: RunOptions = {}) {
  ...
  await Promise.allSettled(
      presets.map(async preset => {
        console.log(`Registering namespace for preset: ${preset.name}`);
        serverInstance.app.log.info(`[Server] Registering namespace for preset: ${preset.name}`);
        return await serverInstance.registerNamespace(`/preset/${preset.name}`, preset.config);
      })
  )
  console.log(`Registered ${presets.length} presets`);
  serverInstance.app.log.info(`[Server] Registered ${presets.length} presets`);
